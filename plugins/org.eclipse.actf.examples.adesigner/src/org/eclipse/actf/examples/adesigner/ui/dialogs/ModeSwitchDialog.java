...
public class ModeSwitchDialog extends TitleAreaDialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		IPerspectiveDescriptor[] perspectiveDesc = _window.getWorkbench()
		...
		modeBaseComp.setBackground(Display.getCurrent().getSystemColor(
				SWT.COLOR_WHITE));
		...
		for (int i = 0; i < modeNum; i++) {
			// if
			// (_window.getWorkbench().getPerspectiveRegistry().findPerspectiveWithId(modePerspectiveIDs[i])
			// != null)
			// {
			final Composite modeComp = new Composite(modeBaseComp, SWT.NONE);
			modeComp.setBackground(Display.getCurrent().getSystemColor(
					SWT.COLOR_WHITE));
			...
			createModeComposite(modeComp, modeNames[i], modeIcons[i],
					modePerspectiveIDs[i]);
			// }
		}

		Composite startupCheckComp = new Composite(dialogArea, SWT.NONE);
		startupCheckComp.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				false, false));
		gridLayout = new GridLayout();
		...
		this._startupCheckButton.setLayoutData(new GridData(SWT.RIGHT,
				SWT.BOTTOM, true, false));
		this._startupCheckButton
				.setText(Messages.ModeSwitchDialog_checkShowDialog);
		this._startupCheckButton
				.setSelection(ADesignerPlugin.getDefault().getPreferenceStore()
						.getString(IPreferenceConstants.STARTUP_OPTION_ID) == IPreferenceConstants.CHOICE_SHOW_MODE_DIALOG);
	...
	private void createModeComposite(Composite parent, String modeName,
			Image iconImage, final String perspectiveID) {
		...
		iconImageLabel.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, true,
				true));
		...
		modeNameLabel.addSelectionListener(new ModeSwitchMouseAdapter(
				perspectiveID));
		modeNameLabel
				.addFocusListener(new ModeSwitchMouseAdapter(perspectiveID));
		modeNameLabel.setBackground(Display.getCurrent().getSystemColor(
				SWT.COLOR_WHITE));
		...
		iconImageLabel.addMouseListener(new ModeSwitchMouseAdapter(
				perspectiveID));
		...
		if (isSelected) {
			changeModeCompColor(iconComp, SWT.COLOR_LIST_SELECTION_TEXT,
	...
	protected void createButtonsForButtonBar(Composite parent) {
		this._okButton = createButton(parent, IDialogConstants.OK_ID,
	...
	protected void checkCache() {
		IPreferenceStore store = ADesignerPlugin.getDefault()
				.getPreferenceStore();
		if (store.getString(IPreferenceConstants.SELECTED_MODE_PERSPECTIVE_ID)
				.equals(IVisualizationPerspective.ID_FLASH_PERSPECTIVE)) {
	...
	protected void okPressed() {
		...
		if (this._startupCheckButton.getSelection()) {
			store.setValue(IPreferenceConstants.STARTUP_OPTION_ID,
					IPreferenceConstants.CHOICE_SHOW_MODE_DIALOG);
		} else {
			store.setValue(IPreferenceConstants.STARTUP_OPTION_ID,
					IPreferenceConstants.CHOICE_DONOT_SHOW_MODE_DIALOG);
		}

		store.setValue(IPreferenceConstants.SELECTED_MODE_PERSPECTIVE_ID,
				this._selectedPerspectiveID);
	...
	private class ModeSwitchMouseAdapter extends MouseAdapter implements
		...
		public void select(EventObject evt) {
			...
			if (null != selectedComp) {
				changeModeCompColor(selectedComp,
						SWT.COLOR_LIST_SELECTION_TEXT, SWT.COLOR_LIST_SELECTION);
				selectedComp.setData(MODE_SELECTED, Boolean.TRUE);

				if (null != _selectedComp
						&& !_selectedComp.equals(selectedComp)) {
					_selectedComp.setData(MODE_SELECTED, Boolean.FALSE);
					changeModeCompColor(_selectedComp, SWT.COLOR_BLACK,
							SWT.COLOR_WHITE);
				}
	...
	private class ModeSwitchMouseTrackAdapter extends MouseTrackAdapter {
		...
		public void deselect(EventObject evt) {
			Composite selectedComp = getSelectedComp(evt);
			if (null != selectedComp) {
				Boolean isSelected = (Boolean) selectedComp
						.getData(MODE_SELECTED);
				if (null != isSelected) {
					if (!isSelected.booleanValue()) {
						changeModeCompColor(selectedComp, SWT.COLOR_BLACK,
								SWT.COLOR_WHITE);
					}
				}
			}
	...
	protected void changeModeCompColor(Composite selectedComp, int iFgColor,
			int iBgColor) {
		Composite selectedCompParent = selectedComp;// selectedComp.getParent();
		selectedCompParent.setBackground(Display.getCurrent().getSystemColor(
				iBgColor));
		Control[] children = selectedCompParent.getChildren();
		for (int i = 0; i < children.length; i++) {
			children[i].setBackground(Display.getCurrent().getSystemColor(
					iBgColor));
			children[i].setForeground(Display.getCurrent().getSystemColor(
					iFgColor));
		}
	}
	...
	protected Composite getSelectedComp(EventObject evt) {
		...
		if (evtSrc instanceof Label) {
			...
			selectedComp = (Composite) ((Link) evtSrc).getParent()
					.getChildren()[0];
		} else {
